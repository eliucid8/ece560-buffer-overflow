...
* **Prepare environment**: In your Linux environment, install nasm and C build tools (“`sudo apt install nasm build-essential`”).
* **Get set up**: Download/clone, extract, and test the example exploit kit.
  * Compile with `make`. The Makefile is set up to build the vulnerable program with W^X disabled. Note: because the `gets` function is dangerously vulnerable, you'll get compiler warnings ("warning: implicit declaration of function ‘gets’", "warning: the `gets' function is dangerous") -- you can ignore these, because vulnerability is our goal. 
...
* **Run demo**: `./demo`
  ...
  * The included attack will make the program skip saying “Bye!” and exit with status code 5 instead of the usual 0.  Recall that the exit status of a program can be checked by running “`echo $?`” after the program exits. The demo script will show this.
